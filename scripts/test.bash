...
# Test platform detection without sourcing utils.bash to avoid shellcheck issues
...
# Test the platform detection logic without actually downloading
test_version="1.1.5"
echo "  🧪 Testing platform detection for version $test_version..."

# Replicate the platform detection logic from utils.bash
get_download_url() {
	local version="$1"
	local OS ARCH DIST
...
if command -v shellcheck >/dev/null 2>&1; then
	# Run shellcheck on individual file groups to avoid external source issues
	lint_failed=false

	if ! shellcheck --shell=bash --external-sources "$plugin_dir"/bin/* "$plugin_dir"/lib/*; then
		lint_failed=true
	fi

	if ! shellcheck --shell=bash "$plugin_dir"/scripts/format.bash "$plugin_dir"/scripts/test.bash; then
		lint_failed=true
	fi
...
if command -v shfmt >/dev/null 2>&1; then
	if shfmt --language-dialect bash --diff "$plugin_dir"/bin/* "$plugin_dir"/lib/* "$plugin_dir"/scripts/* >/dev/null 2>&1; then
